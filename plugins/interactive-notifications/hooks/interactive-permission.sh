...
if command -v jq &> /dev/null; then
    ...
    # Handle AskUserQuestion - show parsed questions with clickable options
    if [ "$TOOL_NAME" = "AskUserQuestion" ]; then
        ...
        if [ "$NUM_QUESTIONS" -eq 0 ]; then
            # No questions, allow tool to proceed
...
" 2>&1)
        ...
        RESPONSES="$RESPONSES}"
        echo "$(date): AskUserQuestion final: $RESPONSES" >> "$LOG_FILE"

        # Deny with answers - this provides answers to Claude without terminal UI
        cat << EOF
