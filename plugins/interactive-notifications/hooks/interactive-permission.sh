...
if command -v jq &> /dev/null; then
    ...
    # Handle AskUserQuestion specially - show parsed questions with options
    if [ "$TOOL_NAME" = "AskUserQuestion" ]; then
        ...
        # Extract questions and build response
        ...
        for ((i=0; i<NUM_QUESTIONS; i++)); do
            ...
            # Build button list (max 3 for AppleScript, use list for more)
            if [ "$NUM_OPTIONS" -le 3 ]; then
                BUTTON_LIST=""
                for ((j=NUM_OPTIONS-1; j>=0; j--)); do
                    LABEL=$(echo "$OPTIONS" | jq -r ".[$j].label // \"Option $((j+1))\"" | head -c 25)
                ...
                DEFAULT_LABEL=$(echo "$OPTIONS" | jq -r '.[0].label // "Option 1"' | head -c 25)
...
" 2>&1)
            ...
            echo "$(date): Q$i [$HEADER]: $RESULT" >> "$LOG_FILE"
            ...
            # Use question text as key for clearer output
            QUESTION_KEY=$(echo "$QUESTION_TEXT" | sed 's/"/\\"/g' | head -c 50)
            RESULT_CLEAN=$(echo "$RESULT" | sed 's/"/\\"/g')
            RESPONSES="$RESPONSES\"$QUESTION_KEY\":\"$RESULT_CLEAN\""
        done

        RESPONSES="$RESPONSES}"
        echo "$(date): Final responses: $RESPONSES" >> "$LOG_FILE"

        # Return deny with answers - Claude will see these and not retry
        cat << EOF
{"hookSpecificOutput":{"hookEventName":"PermissionRequest","decision":{"behavior":"deny","message":"User answered via dialog: $RESPONSES"}}}
EOF
        exit 0
    fi
